...
Before acting, call `skill_search` for curated steps from past runs. If one fits, follow it and call `skill_use` to declare it (so reviews are fast and accurate); if several are close, pick the best; otherwise proceed.
