...
import { addRun } from "../src/skill/store";
import { reviewOutput, assembleMaster } from "../src/skill/reviewer";
...
for (let round = 1; round <= ROUNDS; round++) {
  ...
  lastHaiku = haiku;
  const review = await reviewOutput(skill.id, "haiku", haiku); // persistent session: remembers prior rounds
  const score = review?.score ?? 0;
  scores.push(score);
  addRun({ skillId: skill.id, recipe: "draft | count 5-7-5 | revise", quality: score, review: review ? JSON.stringify(review) : "", ts: round });
  master = (await assembleMaster(skill.id, "haiku")) ?? master;
