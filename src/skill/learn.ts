...
import { enqueueReview, reviewSupervisorActive, transcriptReviewKey } from "./review-queue";
...
interface CopilotReviewContext {
  requests: string[];
  startTs: number;
}
...
function drainReviewQueue(): number {  if (MAX_LEARNERS() <= 0 || reviewSupervisorActive()) return 0;
