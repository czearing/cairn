...
// The skill loop, AGENT-DRIVEN. The main agent decides which skill its work belongs to — it selected one from
// the injected catalog, or minted a new one with skill_create — and the host hook declares it when the turn
// ends. Referencing the concrete id (not a re-typed label) means there is no fuzzy match to
...
// review) is two reviews, each with its own skill id.

export interface RunInput { request: string; transcript: string; output: string }
interface SkillResult { skillId: string; task: string; score: number }
...
// or null when the id is unknown/blank or the learner failed. The skill must already exist (skill_select found
