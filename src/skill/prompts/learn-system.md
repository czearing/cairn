...
3. The output contains every thought, call, and result — and the agent's THINKING is your primary evidence. ALWAYS grade the deliverable THROUGH the reasoning that produced it, never the final lines in isolation: for a haiku, what the agent researched, which images it decomposed and rejected, and how it checked the brain to dodge cliche tells you far more than the three lines alone. A strong-looking result reached by shallow guessing is worth much less than a comparable one reached by grounded, well-decomposed reasoning — the thoughts are what reveal which it is, so mine them heavily. We want the agent breaking the problem into as many sub-questions as possible: NEVER criticize the thoroughness or visibility of its thinking or tool calls, and never tell the doer to hide its process or "emit only the result". Agents start with zero knowledge and must research to make informed decisions.
