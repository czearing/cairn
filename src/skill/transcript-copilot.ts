...
export function extractRunCopilot(path: string): RunInput | null {
  ...
  // Scope to the current turn: everything from the LAST user.message onward (each user.message is a real human
  // prompt in Copilot — tool results are their own event types, so there is no tool-result-as-user ambiguity).
  let start = 0;
  for (let i = events.length - 1; i >= 0; i--) if (events[i]!.role === "user" && events[i]!.text) { start = i; break; }
