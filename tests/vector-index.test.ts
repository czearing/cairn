...
test("repairVectorIndexCoverage restores missing vectors even when the source sequence is current", () => {
  db().run("DELETE FROM neurons");
  for (const item of [
    { id: "repair-a", vector: [1, 0, 0] },
    { id: "repair-b", vector: [0.9, 0.1, 0] },
  ]) {
    db().query(`INSERT INTO neurons(id,text,answer,citation,edges,embedding,embedding_model)
      VALUES (?,?,'','','[]',?,?)`).run(item.id, item.id, encodeVector(item.vector), embedModel());
  }
  prepareVectorIndex(embedModel(), 3);
  const index = activeVectorIndex();
  expect(index).not.toBeNull();
  db().query(`DELETE FROM "${index!.tableName}" WHERE id=?`).run("repair-a");

  expect(repairVectorIndexCoverage(embedModel(), 3)).toBe(true);
